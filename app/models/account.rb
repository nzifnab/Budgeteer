class Account < ActiveRecord::Base
    ...
    if self.should_distribute?
      ...
      if amt > 0
        ...
        if self.has_reached_cap?
          ...
          if this_left > 0
            if self.does_overflow? && self.has_fulfilled_prerequisite?
  ...
  def should_distribute?
    return (self.enabled && self.has_fulfilled_prerequisite?)
  end

  def has_fulfilled_prerequisite?
    ((self.has_prerequisite? && self.prerequisite.amount >= self.prerequisite.cap) || !self.has_prerequisite?)
  end
